...
def euler_integrate(fun, x0, time_max, time_step):
    ...
    x = np.zeros([len(time), len(x0)])
    x[0] = float(x0)
    for i, ti in enumerate(time[:-1]): # time[:-1] = copy of array "time", w/o the last element
...
def ode_integrate_rk(fun_rk, x0, time_max, time_step):
    ...
    solver.set_integrator('dopri5')
    solver.set_initial_value(x0, time[0])
    for i, ti in enumerate(time[:]): # time[:] = copy of the array time, with all elements
