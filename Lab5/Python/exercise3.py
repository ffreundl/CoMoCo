...
####################################### End OF 3a ######################################## 
    ...
    # Define Muscle Attachment points
    m1_origin = np.array([-0.15, 0.0])  # Origin of Muscle 1
    m1_insertion = np.array([0.0, -0.20])  # Insertion of Muscle 1

    m2_origin = np.array([0.15, 0.0])  # Origin of Muscle 2
    m2_insertion = np.array([0.0, -0.05])  # Insertion of Muscle 2
    ...
    act1=np.ones((len(time),1))*0.20
    act2=np.ones((len(time),1))*0.20
    ...
    plt.figure('Pendulum')
    plt.title('Pendulum Phase for a null activation - passive muscles')
